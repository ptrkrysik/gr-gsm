...
{
    return gnuradio::get_initial_sptr(new burst_source_impl(framenumbers, timeslots, burst_data));
}
...
burst_source_impl::burst_source_impl(const std::vector<int>& framenumbers,
                                     const std::vector<int>& timeslots,
                                     const std::vector<std::string>& burst_data)
    : gr::block("burst_source", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)),
      d_finished(false),
      d_arfcn(0)
{
...
void burst_source_impl::set_framenumbers(const std::vector<int>& framenumbers) { d_framenumbers = framenumbers; }

void burst_source_impl::set_timeslots(const std::vector<int>& timeslots) { d_timeslots = timeslots; }

void burst_source_impl::set_burst_data(const std::vector<std::string>& burst_data) { d_burst_data = burst_data; }

void burst_source_impl::set_arfcn(uint16_t arfcn) { d_arfcn = arfcn; }

bool burst_source_impl::start()
{
    d_finished = false;
    d_thread = std::shared_ptr<gr::thread::thread>(new gr::thread::thread(boost::bind(&burst_source_impl::run, this)));
    return block::start();
}
...
bool burst_source_impl::finished() { return d_finished; }

void burst_source_impl::run()
{
    for (int i = 0; i < d_burst_data.size(); i++) {
        if (d_burst_data[i].length() == BURST_SIZE && d_timeslots[i] >= 0 && d_timeslots[i] <= 7 &&
