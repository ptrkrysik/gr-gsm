...
void extract_system_info_impl::process_sysinfo(pmt::pmt_t msg)
{
    ...
    if (msg_elements[2] == 0x1b) {
        ...
        info.mcc = ((msg_elements[5] & 0xF) * 100) + (((msg_elements[5] & 0xF0) >> 4) * 10) +
                   ((msg_elements[6] & 0xF));                             // take mcc
        ...
        info.mcc = ((msg_elements[3] & 0xF) * 100) + (((msg_elements[3] & 0xF0) >> 4) * 10) +
                   ((msg_elements[4] & 0xF));                             // take mcc
...
extract_system_info::sptr extract_system_info::make()
{
    return gnuradio::get_initial_sptr(new extract_system_info_impl());
}
...
extract_system_info_impl::extract_system_info_impl()
    : gr::block("extract_system_info", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)),
      after_reset(false)
{
    message_port_register_in(pmt::mp("bursts"));
    set_msg_handler(pmt::mp("bursts"),
                    boost::bind(&extract_system_info_impl::process_bursts, this, boost::placeholders::_1));
    message_port_register_in(pmt::mp("msgs"));
    set_msg_handler(pmt::mp("msgs"),
                    boost::bind(&extract_system_info_impl::process_sysinfo, this, boost::placeholders::_1));
}
