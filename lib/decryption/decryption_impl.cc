...
decryption::sptr decryption::make(const std::vector<uint8_t>& k_c, unsigned int a5_version)
{
    return gnuradio::get_initial_sptr(new decryption_impl(k_c, a5_version));
}
...
decryption_impl::decryption_impl(const std::vector<uint8_t>& k_c, unsigned int a5_version)
    : gr::block("decryption", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)), d_k_c_valid(false)
{
    set_k_c(k_c);
...
void decryption_impl::set_k_c(const std::vector<uint8_t>& k_c) { d_k_c = k_c; }
