...
public:
    Generator(uint64_t wCoeff, unsigned wLen)
        : mCoeff(wCoeff), mState(0), mMask((1ULL << wLen) - 1), mLen(wLen), mLen_1(wLen - 1)
    {
        assert(wLen < 64);
    }
...
public:
    Parity(uint64_t wCoefficients, unsigned wParitySize, unsigned wCodewordSize)
        : Generator(wCoefficients, wParitySize), mCodewordSize(wCodewordSize)
    {
    }
...
public:
    ...
    BitVector alias() const { return const_cast<BitVector*>(this)->segment(0, size()); }
    ...
    void dup(BitVector other)
    {
        assert(!this->getData());
        makeAlias(other);
        assert(this->mStart == other.mStart);
    }
    void dup(BitVector& other)
    {
        makeAlias(other);
...
public:
    ...
    */
    SoftVector(float* wData, unsigned length) : Vector<float>(wData, length) {}

    SoftVector(float* wData, float* wStart, float* wEnd) : Vector<float>(wData, wStart, wEnd) {}
