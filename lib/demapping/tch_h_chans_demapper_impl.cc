...
tch_h_chans_demapper::sptr tch_h_chans_demapper::make(unsigned int timeslot_nr, unsigned int tch_h_channel)
{
    return gnuradio::get_initial_sptr(new tch_h_chans_demapper_impl(timeslot_nr, tch_h_channel));
}
...
tch_h_chans_demapper_impl::tch_h_chans_demapper_impl(unsigned int timeslot_nr, unsigned int tch_h_channel)
    : gr::block("tch_h_chans_demapper", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)),
...

{
    ...
    set_msg_handler(pmt::mp("bursts"),
                    boost::bind(&tch_h_chans_demapper_impl::filter_tch_chans, this, boost::placeholders::_1));
...
void tch_h_chans_demapper_impl::filter_tch_chans(pmt::pmt_t msg)
{
    ...
    if (uplink_burst) {
        sacch_tch_demapper(fn_mod13,
                           fn_mod26,
                           frame_nr,
                           d_bursts_sacch_ul,
                           d_frame_numbers_sacch_ul,
                           d_bursts_ul,
                           d_frame_numbers_ul,
                           msg_out);
    } else {
...
void tch_h_chans_demapper_impl::sacch_tch_demapper(uint32_t fn_mod13,
                                                   u_int32_t fn_mod26,
                                                   uint32_t frame_nr,
        ...
        if (our_sub) {
            ...
            if (fn_mod13 == 1 || fn_mod13 == 3 || fn_mod13 == 5) {
                ...
                for (int jj = 1; jj < 4; jj++) {
                    ...
                    if (((d_frame_numbers[tch_burst_nr][jj] - d_frame_numbers[tch_burst_nr][jj - 1]) != 2) &&
