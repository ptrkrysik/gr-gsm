...
private:
    ...
    std::vector<int> d_cell_allocation; ///< stores cell allocation - absolute rf channel numbers (ARFCNs) assigned to
                                        ///< the given cell. The variable should at least contain C0 channel number.
    //@}

    gr_complex d_sch_training_seq[N_SYNC_BITS]; ///< encoded training sequence of a SCH burst
    gr_complex d_norm_training_seq[TRAIN_SEQ_NUM]
     ...
     */
    void
    detect_burst(const gr_complex* input, gr_complex* chan_imp_resp, int burst_start, unsigned char* output_binary);
     ...
     */
    void send_burst(burst_counter burst_nr,
