...
controlled_rotator_cc::sptr controlled_rotator_cc::make(double phase_inc)
{
    return gnuradio::get_initial_sptr(new controlled_rotator_cc_impl(phase_inc));
}
...
void controlled_rotator_cc_impl::set_phase_inc(double phase_inc)
{
...
int controlled_rotator_cc_impl::work(int noutput_items,
                                     ...
                                     gr_vector_void_star& output_items)
{
    ...
    /*if(input_items.size() == 2) {
      ...
      while(ii < noutput_items){
        ...
        if(d_phase_inc != (*pp)){
          ...
          float freq_offset_setting = (*(pp) / (2*M_PI)) * d_samp_rate; //send stream tag with a new value of the
    frequency offset
    ...
    */
    ...
    for (std::vector<tag_t>::iterator i_tag = set_phase_inc_tags.begin(); i_tag < set_phase_inc_tags.end(); i_tag++) {
        ...
        //
        //        float freq_offset_setting = (pmt::to_double(i_tag->value) / (2*M_PI)) * d_samp_rate; //send stream tag
        //        with a new value of the frequency offset pmt::pmt_t key =
        //        pmt::string_to_symbol("setting_freq_offset"); pmt::pmt_t value =
        //        pmt::from_double(freq_offset_setting); add_item_tag(0,i_tag->offset, key, value);
    }

    d_r.rotateN((out + produced_out),
                const_cast<gr_complex*>(in + processed_in),
                (noutput_items - produced_out)); // const_cast<gr_complex *> is workaround old implementation of rotateN
                                                 // that is still present in ubuntu 14.04 packages
