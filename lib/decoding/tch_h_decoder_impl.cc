...
tch_h_decoder::sptr tch_h_decoder::make(unsigned int sub_channel, std::string multi_rate, bool boundary_check)
{
    return gnuradio::get_initial_sptr(new tch_h_decoder_impl(sub_channel, multi_rate, boundary_check));
}
...
tch_h_decoder_impl::tch_h_decoder_impl(unsigned int sub_channel, std::string multi_rate, bool boundary_check)
    : gr::block("tch_h_decoder", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)),
      ...
      d_cmr(0)
{
    ...
    if (multi_rate.length()) {
        ...
        for (std::string::const_iterator it = multi_rate.begin(); it != multi_rate.end(); it += 2) {
        ...
        //         .... 1... = ICMI: Initial Codec Mode Indicator: The initial codec mode is defined by the Start Mode
        //         field (1)
        //         .... ..00 = Start Mode: 0
        ...
        for (std::vector<uint8_t>::const_iterator it = d_multi_rate_codes.begin(); it != d_multi_rate_codes.end();
             it++) {
            switch (*it) {
            case 0:
...
void tch_h_decoder_impl::decode(pmt::pmt_t msg)
{
    ...
    if (d_tch_mode == TCH_HS) {
        ...
        frameLength = gsm0503_tch_ahs_decode(frameBuffer,
                                             bursts_s,
                                             fn_is_odd,
                                             fn_is_odd, // int codec_mode_req,
                                             &d_multi_rate_codes.front(),
                                             d_multi_rate_codes.size(),
                                             &d_ft,
                                             &d_cmr,
                                             &n_errors,
                                             &n_bits_total);
    ...
    if (!n_errors && (!d_boundary_check || d_boundary_decode)) {
        ...
        if (d_tch_mode != TCH_HS) {
            ...
            case 20:
                ...
                break; // TCH/AHS7.95
            default:
                std::cerr << "Unexpected voice frame length:" << frameLength << std::endl;
                return;
            }
            frameLength += 1;
        }
