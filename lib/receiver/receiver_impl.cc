...
{
    return gnuradio::get_initial_sptr(new receiver_impl(osr, cell_allocation, tseq_nums, process_uplink));
}

/* The private constructor */
receiver_impl::receiver_impl(int osr,
                             const std::vector<int>& cell_allocation,
                             const std::vector<int>& tseq_nums,
                             bool process_uplink)
    : gr::sync_block("receiver", gr::io_signature::make(1, -1, sizeof(gr_complex)), gr::io_signature::make(0, 0, 0)),
...
void receiver_impl::fcch_search_handler(const gr_complex* input, int noutput_items)
{
    ...
    pmt::pmt_t msg = pmt::make_tuple(
        pmt::mp("freq_offset"), pmt::from_double(freq_offset_tmp - d_freq_offset_setting), pmt::mp("fcch_search"));
...
#pragma omp parallel for
        for (size_t input_nr = 0; input_nr < inputs_to_process; input_nr++) {
            ...
            case fcch_burst: {
                ...
                double freq_offset_tmp = compute_freq_offset(input, first_sample, last_sample);
            ...
            case sch_burst: {
                ...
                if (rc) {
                    if (++d_failed_sch >= MAX_SCH_ERRORS) {
                        ...
                        /* Frequency correction loop */
                        pmt::pmt_t msg =
                            pmt::make_tuple(pmt::mp("freq_offset"), pmt::from_double(0.0), pmt::mp("sync_loss"));
                        message_port_pub(pmt::mp("measurements"), msg);
                    }
            ...
            case normal_or_noise: {
                ...
                tseq_num = input_nr <= d_tseq_nums.size() ? d_tseq_nums[input_nr - 1] : d_tseq_nums.back();
...
bool receiver_impl::find_fcch_burst(const gr_complex* input, const int nitems, double& computed_freq_offset)
{
    ...
    boost::circular_buffer<float> phase_diff_buffer(FCCH_HITS_NEEDED * d_OSR);
        ...
        case found_something: {
            ...
            if ((miss_count < FCCH_MAX_MISSES * d_OSR) && (hit_count > FCCH_HITS_NEEDED * d_OSR)) {
                ...
                if (lowest_max_min_diff > max_phase_diff - min_phase_diff) {
                    ...
                    for (buffer_iter = phase_diff_buffer.begin(); buffer_iter != (phase_diff_buffer.end());
                         buffer_iter++) {
...
bool receiver_impl::reach_sch_burst(const int nitems)
{
    ...
    int to_consume = 0;
    bool result = false;
    unsigned sample_nr = d_fcch_start_pos + (FRAME_BITS - SAFETY_MARGIN) * d_OSR;

    /* Consume samples until d_counter will be equal to sample_nr */
    if (d_counter < sample_nr) {
        to_consume = d_counter + nitems >= sample_nr ? sample_nr - d_counter : nitems;
    } else {
        to_consume = 0;
        result = true;
    }
...
int receiver_impl::get_sch_chan_imp_resp(const gr_complex* input, gr_complex* chan_imp_resp)
{
    ...
    strongest_window_nr =
        max_element(window_energy_buffer.begin(), window_energy_buffer.end()) - window_energy_buffer.begin();
...
void receiver_impl::detect_burst(const gr_complex* input,
...
inline void
receiver_impl::mafi(const gr_complex* input, int nitems, gr_complex* filter, int filter_length, gr_complex* output)
{
...
int receiver_impl::get_norm_chan_imp_resp(const gr_complex* input, gr_complex* chan_imp_resp, float* corr_max, int bcc)
{
    ...
    for (int ii = search_start_pos; ii < search_stop_pos; ii++) {
        gr_complex correlation =
            correlate_sequence(&d_norm_training_seq[bcc][TRAIN_BEGINNING], N_TRAIN_BITS - 10, &input[ii]);
        correlation_buffer.push_back(correlation);
        power_buffer.push_back(std::pow(abs(correlation), 2));
    }
...
void receiver_impl::send_burst(burst_counter burst_nr,
                               const unsigned char* burst_binary,
                               uint8_t burst_type,
                               size_t input_nr,
                               unsigned int burst_start)
{
    ...
    if (dl_burst) {
        ...
        tn = static_cast<uint8_t>(d_burst_nr.subtract_timeslots(3).get_timeslot_nr());
        frame_number = htobe32(d_burst_nr.subtract_timeslots(3).get_frame_nr());
        arfcn = htobe16(d_cell_allocation[input_nr] | GSMTAP_ARFCN_F_UPLINK);
    }
...
void receiver_impl::configure_receiver(void)
{
    ...
    d_channel_conf.set_burst_types(
        TIMESLOT0, TEST_CCH_FRAMES, sizeof(TEST_CCH_FRAMES) / sizeof(unsigned), dummy_or_normal);
...
void receiver_impl::set_tseq_nums(const std::vector<int>& tseq_nums) { d_tseq_nums = tseq_nums; }

void receiver_impl::reset(void) { d_state = fcch_search; }
