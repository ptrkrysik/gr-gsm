...
message_source::sptr message_source::make(const std::vector<std::string>& msg_data)
{
    return gnuradio::get_initial_sptr(new message_source_impl(msg_data));
}
...
message_source_impl::message_source_impl(const std::vector<std::string>& msg_data)
    : gr::block("message_source", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)), d_finished(false)
{
...
bool message_source_impl::start()
{
    d_finished = false;
    d_thread =
        std::shared_ptr<gr::thread::thread>(new gr::thread::thread(boost::bind(&message_source_impl::run, this)));
    return block::start();
}
...
bool message_source_impl::finished() { return d_finished; }
