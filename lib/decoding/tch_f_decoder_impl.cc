...
tch_f_decoder::sptr tch_f_decoder::make(tch_mode mode, bool boundary_check)
{
    return gnuradio::get_initial_sptr(new tch_f_decoder_impl(mode, boundary_check));
}
...
tch_f_decoder_impl::tch_f_decoder_impl(tch_mode mode, bool boundary_check)
    : gr::block("tch_f_decoder", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)),
      d_tch_mode(mode),
...
void tch_f_decoder_impl::decode(pmt::pmt_t msg)
{
    ...
    if (d_collected_bursts_num == 8) {
        ...
        for (int ii = 0; ii < 8; ii++) {
            ...
            if ((ii <= 3 && static_cast<int>(burst_bits[87]) == 1) ||
                (ii >= 4 && static_cast<int>(burst_bits[60]) == 1)) {
                stolen = true;
            }
        }
        ...
        if (d_tch_mode == TCH_FS || d_tch_mode == TCH_EFR) {
            ...
            if (good) {
                ...
                } else if (d_tch_mode == TCH_EFR) // GSM-EFR
                {
                    ...
                    // when itegrating with libosmocore lines above can be removed and line below uncommented, efr
                    // decoding with libosmocore need to be tested however gsm0503_tch_fr_decode(frameBuffer, bursts_s,
                    // 1, 1, &n_errors, &n_bits_total);
                }
                message_port_pub(pmt::mp("voice"),
                                 pmt::cons(pmt::PMT_NIL, pmt::make_blob(frameBuffer, mTCHFrameLength)));
            }
        } else {
