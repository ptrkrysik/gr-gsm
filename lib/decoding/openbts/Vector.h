...
template <class T>
class RCData : public RefCntBase
{
public:
    T* mPointer;
};
...
//              The const copy-constructor did a clone, the non-const copy constructor did a shiftMem, and the segment
//              and related methods (head, tail, etc) returned aliases. Since a copy-constructor is inserted
//              transparently in sometimes surprising places, this made the class very difficult to use.  Moreover,
//              since the C++ standard specifies that a copy-constructor is used to copy the return value from
//              functions, it makes it literally impossible for a function to fully control the return value.  Our code
//              has relied on the "Return Value Optimization" which says that the C++ compiler may omit the
//              copy-construction of the return value even if the copy-constructor has side-effects, which ours does.
//              This methodology is fundamentally incompatible with C++.
// What the original behavior actually was:
//      class Vector:
//              The copy-constructor and assignment operators did a clone for the const case and a shift for the
//              non-const case.
...
//      class BitVector:
//              Copy-constructor:
//                      BitVector did not have any copy-constructors, and I think the intent was that it would have the
//                      same behavior as Vector, but that is not how C++ works: with no copy-constructor the default
//                      copy-constructor uses only the const case, so only the const Vector copy-constructor was used.
//                      Therefore it always cloned, and the code base relied heavily on the "Return Value Optimization"
//                      to work at all.
//              Assignment operator:
//                      BitVector did not have one, so C++ makes a default one that calls Vector::operator=() as a side
//                      effect, which did a clone; not sure if there was a non-const version and no longer care.
//              segment methods:
//                      The non-const segment() returned an alias, and the const segment() returned a clone.
//                      I think the intent was that the behavior should be the same as Vector, but there was a
//                      conversion of the result of the const segment() method from Vector to BitVector which caused the
//                      Vector copy-constructor to be (inadvertently) invoked, resulting in the const version of the
//                      segment method returning a clone.
...
//              The VectorBase class MUST NOT CONTAIN: copy constructors, non-trivial constructors called from derived
//              classes, or any method that returns a VectorBase type object.  Why?  Because any of the above when used
//              in derived classes can cause copy-constructor invocation, often surprisingly, obfuscating the code. Each
//              derived class must provide its own: copy-constructors and segment() and related methods, since we do not
//              want to inadvertently invoke a copy-constructor to convert the segment() result from VectorBase to the
//              derived type.
//      BitVector:
...
//              If you want a clone of a segment, use cloneSegment(), which replaces the previous: const segment(...)
//              const method. Note that the semantics of cloneSegment still rely on the Return Value Optimization.  Oh
//              well, we should use refcnts.
//      Vector:
//              I left Vector alone (except for rearrangement to separate out VectorBase.)  Vector should just not be
//              used.
//      SoftVector:
...
protected:
    ...
    // Init vector with specified size.  Previous contents are completely discarded.  This is only used for
    // initialization.
