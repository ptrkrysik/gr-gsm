...
control_channels_decoder::sptr control_channels_decoder::make()
{
    return gnuradio::get_initial_sptr(new control_channels_decoder_impl());
}
...
control_channels_decoder_impl::control_channels_decoder_impl()
    : gr::block("control_channels_decoder", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0)),
      d_collected_bursts_num(0)
{
    // setup input/output ports
    message_port_register_in(pmt::mp("bursts"));
    set_msg_handler(pmt::mp("bursts"),
                    boost::bind(&control_channels_decoder_impl::decode, this, boost::placeholders::_1));
...
void control_channels_decoder_impl::decode(pmt::pmt_t msg)
{
    ...
    if (d_collected_bursts_num == 4) {
        ...
        if (gsm0503_xcch_decode(result, bursts_s, &n_errors, &n_bits_total) != -1) {
            ...
            // prepare message
            pmt::pmt_t msg_out =
                pmt::cons(pmt::PMT_NIL, pmt::make_blob(header_plus_data, DATA_BYTES + sizeof(gsmtap_hdr)));
