...
void tmsi_dumper_impl::dump_tmsi(pmt::pmt_t msg)
{
    ...
    if (direction_and_protocol ==
            0x06 && // direction from originating site, transaction id==0, Radio Resouce Management protocol
        ...
        case 0x21: // Paging Request Type 1
        {
            ...
            if (found_id_element == true) {
                ...
                if ((next_element_index < (msg_len + 1)) && (element_id == 0x17)) {
                    ...
                    if (mobile_identity_type == 0x04) // identity type: TMSI
                    {
                        write_tmsi(m + next_element_index +
                                   3); // write starting from position of the TMSI in the message
                        dump_file << "-";
                        write_timestamp(now);
                        dump_file << "-0";
                        dump_file << std::endl;
                    } else if (mobile_identity_type == 0x01) // identity type: IMSI
                    {
                        ...
                        write_timestamp(now);
                        dump_file << "-";
                        write_imsi(m + next_element_index +
                                   2); // write starting from position of the IMSI in the message
                        dump_file << std::endl;
                    }
                }
                int ii;
...
inline void tmsi_dumper_impl::write_timestamp(tm* now)
{
    dump_file << boost::format("%d%02d%02d%02d%02d%02d") %
                     (now->tm_year + 1900 - 2000) // year -2000 here after the 1900 leaves you with 15 instead of 2015
                                                  // (delivery reports format is 150112223501)
                     % (now->tm_mon + 1)                         // month
                     % now->tm_mday                              // day
...
{
    dump_file << boost::format("%1x%02x%02x%02x%02x%02x%02x%02x") % (swap(imsi[0]) & 0x0f) % swap(imsi[1]) %
                     swap(imsi[2]) % swap(imsi[3]) % swap(imsi[4]) % swap(imsi[5]) % swap(imsi[6]) % swap(imsi[7]);
    return;
}

void tmsi_dumper_impl::write_tmsi(uint8_t* tmsi)
{
    dump_file << boost::format("%02x%02x%02x%02x") % (int)tmsi[0] % (int)tmsi[1] % (int)tmsi[2] % (int)tmsi[3];
    return;
}


tmsi_dumper::sptr tmsi_dumper::make() { return gnuradio::get_initial_sptr(new tmsi_dumper_impl()); }

/*
 * The private constructor
 */
tmsi_dumper_impl::tmsi_dumper_impl()
    : gr::block("tmsi_dumper", gr::io_signature::make(0, 0, 0), gr::io_signature::make(0, 0, 0))
{
 ...
 */
tmsi_dumper_impl::~tmsi_dumper_impl() { dump_file.close(); }
} /* namespace gsm */
} /* namespace gr */
